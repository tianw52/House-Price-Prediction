...
* UW ID: 20934015
* Kaggle public score: 0.20101
* Kaggle submission count/times: 17
...
set.seed(444)
train_control <- trainControl(method="cv", number=5)
tuneGrid <- expand.grid(.mtry=c( 9, 10, 12, 20, 25, 30, 33),
                        .min.node.size = c(2, 3, 4, 5, 8, 10),
                        .splitrule = c("variance", "extratrees"))

tr2 = train(
  x = dtrain_full[ , select_var],
  y = dtrain_full[ , names(dtrain_full) == 'price'],
  method = 'ranger', trControl = train_control, tuneGrid = tuneGrid
)
...
```{r, eval=FALSE, echo=FALSE}
set.seed(444) 
m1 <- ranger(price ~ saledate + gba + saleyear + landarea + build_age+
                     buy_first+ nbhd + eyb+ cndtn + total_bath + latitude
                     + longitude+ grade + ayb + + rooms + avg_room_size
             ,data=dtrain_full[id.train,],
              mtry = 12, min.node.size = 3, splitrule = "extratrees")
pred <- predict(m1, test)$prediction
actual <- test$price
# estimated APSE 
mean((actual-pred)^2)
```
...
```{r, eval=FALSE, echo=FALSE}
set.seed(444) 
m2 <- randomForest(price ~ saledate + gba + saleyear + landarea + build_age+
                     buy_first+ nbhd + eyb+ cndtn + total_bath + latitude + yr_rmdl 
                     + longitude+ grade + ayb + + rooms + avg_room_size + ward + quadrant
             , data=dtrain_full[id.train,])
...
```{r, eval=FALSE, echo=FALSE}
set.seed(444) 
m3 <- randomForest(price ~ saledate + gba + saleyear + landarea + build_age+
                     buy_first+ nbhd + eyb+ cndtn + total_bath + latitude + yr_rmdl 
                     + longitude+ grade + ayb + + rooms + avg_room_size + ward + quadrant
             ,data=dtrain_full[id.train,],
              mtry = 6, nodesize = 4)
...
```{r, eval=FALSE, echo=FALSE}
set.seed(444) 
m4 <- randomForest(price ~ saledate + gba + saleyear + landarea + build_age+
                     buy_first+ nbhd + eyb+ cndtn + total_bath + latitude + yr_rmdl 
                     + longitude+ grade + ayb + + rooms + avg_room_size + ward + quadrant
             ,data=dtrain_full[id.train,],
              mtry = 6, nodesize = 3)
...
```{r, eval=FALSE, echo=FALSE}
# Function to tune ranger
tuneRangerModel <- function(train_data, test_data, formula, 
                            mtry_values, min_node_size_values, max_depth_values, split) {
  set.seed(444)
  
  best_model <- NULL
  best_mtry <- NA
  best_min_node_size <- NA
  best_max_depth <- NA
  lowest_apse <- Inf 
    ...
    for (min_node_size in min_node_size_values) {
      for (max_depth in max_depth_values) {
        for (s in split) {
          ...
          apse <- mean((actual - predictions)^2)
  ...
  list(best_model = best_model,
       ...
       lowest_apse = lowest_apse,
       best_split = best_split)
}


result <- tuneRangerModel(dtrain_full[id.train, ], test,
                          price ~ saledate + gba + saleyear + landarea + 
                                  buy_first + nbhd + eyb + cndtn + total_bath + 
                                  latitude + yr_rmdl + longitude + grade + ayb+
                                  + rooms + avg_room_size +build_age + ward + quadrant,
                          mtry_values = c(4, 5, 6),
                          min_node_size_values = c(2, 3, 4, 5, 6),
                          max_depth_values = c(NULL, 5, 7, 10, 15),
                          split = c("variance", "extratrees"))

# Print the results
print(result$best_mtry)
print(result$best_min_node_size)
print(result$best_max_depth)
print(result$best_split)
print(result$lowest_apse)
```
...
<!-- # ========== model ================ -->
<!--   # fit <- ranger((price^0.101-1)/0.101 ~  -->
<!--   #                 saledate + gba + saleyear + landarea +  -->
<!--   #                 buy_first + nbhd + eyb + cndtn + total_bath +  -->
<!--   #                 latitude + yr_rmdl + longitude + grade + ayb+ -->
<!--   #                 + rooms + avg_room_size + build_age + ward + quadrant, -->
<!--   #              data=dtrain_full)  -->
...
<!--   # 0.20810 -->
<!--   # fit <- ranger((price^0.101-1)/0.101 ~ -->
<!--   #                 saledate + gba + saleyear + landarea + build_age+ -->
<!--   #                 buy_first+ nbhd + eyb+ cndtn + total_bath + latitude -->
<!--   #               + longitude+ grade + ayb  + rooms + avg_room_size, -->
<!--   #               data = dtrain_full, mtry = 12, splitrule = "extratrees",  -->
<!--   #               min.node.size = 5) -->
<!--   # 0.20741 -->
<!--   # fit <- ranger((price^0.101-1)/0.101 ~ -->
<!--   #                 saledate + gba + saleyear + landarea + build_age+ -->
<!--   #                 buy_first+ nbhd + eyb+ cndtn + total_bath + latitude -->
<!--   #               + longitude+ grade + ayb  + rooms + avg_room_size + bedrm, -->
<!--   #               data = dtrain_full, mtry = 9, -->
<!--   #               min.node.size = 3) -->
<!--   # 0.20905 -->
<!--   # fit <- ranger((price^0.101-1)/0.101 ~ -->
<!--   #                 saledate + gba + saleyear + landarea + -->
<!--   #                 buy_first + nbhd + eyb + cndtn + total_bath + -->
<!--   #                 latitude + longitude + grade + ayb +  -->
<!--   #                 + rooms + avg_room_size + build_age + ward + quadrant, -->
<!--   #              data=dtrain_full) -->
<!--   # 0.20692 -->
<!--   # fit <- ranger((price^0.101-1)/0.101 ~ -->
...
<!--   #               min.node.size = 2) -->
<!--   # 0.20516 -->
<!--   # now with 4 interaction -->
<!--   # fit <- ranger((price^0.101-1)/0.101 ~ ., -->
<!--   #               data = dtrain_full, -->
<!--   #               mtry = 14, splitrule = "variance", -->
<!--   #               min.node.size = 5) -->
<!--   # 0.20278 -->
<!--   # fit <- ranger((price^0.101-1)/0.101 ~ ., -->
<!--   #               data = dtrain_full, -->
<!--   #               mtry = 30, splitrule = "extratrees", -->
<!--   #               min.node.size = 2) -->
<!--   # 0.20115 -->
<!--   ## 6 interaction -->
<!--   # fit <- ranger((price^0.101-1)/0.101 ~ ., -->
