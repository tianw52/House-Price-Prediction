---
title: "STAT 444 Project 2 -- Smoothing Model"
author: "Tianyi Wu"
output: 
    pdf_document
---
...
cross_val <- function(data, formula, k = 5, lambda, error_metric = 'RMSLE') {
  set.seed(42)
  # Create k folds
  folds <- createFolds(data$price, k = k, list = TRUE, returnTrain = FALSE)
  
  # Initialize a vector to store the errors for each fold
  errors <- numeric(k)
  ...
  for(i in seq_along(folds)) {
    ...
    predictions_transformed <- predict(gam_model, newdata = test_set)
    
    
    # Transform predictions back to the original scale
    predictions_original <- ifelse(lambda == 0,
                                   exp(predictions_transformed),
                                   (predictions_transformed * lambda + 1)^(1 / lambda))
    
    # Compute the error for the validation set based on the specified metric
    
    log_preds <- log(predictions_original + 1)
    log_actuals <- log(test_set$price + 1)
    errors[i] <- sqrt(mean((log_preds - log_actuals)^2))
...
```{r, eval=F,echo=F}
dtest$intwall <- as.character(dtest$intwall)
for (i in seq(nrow(dtest))) {
  if (dtest$intwall[i] == "Default") {
    dtest$intwall[i] <- "Hardwood"
  }
}
dtest$intwall <- as.factor(dtest$intwall)

dtest$style <- as.character(dtest$style)
for (i in seq(nrow(dtest))) {
  if (dtest$style[i] == "Default") {
    dtest$style[i] <- "2 Story"
  }
}
dtest$style <- as.factor(dtest$style)
```
